...
func IndentSpace(dst *bytes.Buffer, src []byte, prefix, indent string) error {
	origLen := dst.Len()
	err := IndentStreamSpace(dst, bytes.NewBuffer(src), prefix, indent)
	if err != nil {
		dst.Truncate(origLen)
	}
	return err
...
// IndentStream is an implementation of PPrint for generic readers and writers
func IndentStreamSpace(dst io.Writer, src io.Reader, prefix, indent string) error {
	var lex lexer
	lex.reset()
	tokStack := newTokenStack()
	curType := tokenError
